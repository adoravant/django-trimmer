...
class BTemplate(object):
	...
	def __init__(self, template, project):
		self.template = f"C:/Users/Abrahan/Desktop/django-trimmer/trim/__BOOTSTRAP/{template}"
	...
	def send_main_html(self):
		...
		for root, dirs, files in os.walk(self.template, topdown=False):
			for name in files:
				if name.endswith("html"):
					html.append(name)	
					if os.path.exists(f"pytharg/templates/{self.project}/main/{name}") == True:
						if os.path.getsize(f"pytharg/templates/{self.project}/main/{name}") >= \
						os.path.getsize(f"{os.path.join(root, name)}"):
							pass
						else:
							os.remove(f"pytharg/templates/{self.project}/main/{name}")
							shutil.copy(f"{os.path.join(root, name)}", f"pytharg/templates/{self.project}/main/{name}")
	...
	def required_css_files(self):
		...
		for html in self.send_main_html():
			soup = BeautifulSoup(open(f"pytharg/templates/{self.project}/main/{html}"), features="html.parser")
		...
		for root, dirs, files in os.walk(self.template, topdown=False):
			for name in files:
				...
				if ( name.endswith("js")) and (name in required_js):
					if os.path.exists(f"pytharg/static/{self.project}/js/{name}") == True:
						if os.path.getsize(f"pytharg/static/{self.project}/js/{name}") >= \
						os.path.getsize(f"{os.path.join(root, name)}"):
							pass
						else:
							os.remove(f"pytharg/static/{self.project}/js/{name}")
							shutil.copy(f"{os.path.join(root, name)}", f"pytharg/static/{self.project}/js/{name}")
							js.append(name)
					else:
						js.append(name)
						shutil.copy(f"{os.path.join(root, name)}", f"pytharg/static/{self.project}/js/{name}")							
		...
		for root, dirs, files in os.walk(self.template, topdown=False):
			for name in files:
				if (name.endswith("css") and name in required_css):
					if os.path.exists(f"pytharg/static/{self.project}/css/{name}") == True:
						if os.path.getsize(f"pytharg/static/{self.project}/css/{name}") >= \
						os.path.getsize(f"{os.path.join(root, name)}"):
							pass
						else:
							os.remove(f"pytharg/static/{self.project}/css/{name}")
							shutil.copy(f"{os.path.join(root, name)}", f"pytharg/static/{self.project}/css/{name}")
							css.append(name)
					else:
						css.append(name)
						shutil.copy(f"{os.path.join(root, name)}", f"pytharg/static/{self.project}/css/{name}")							
		...
		for root, dirs, files in os.walk(self.template, topdown=False):
			for name in files:
				if name.endswith(("svg", "png", "jpg", "jpeg", "gif")):
					if os.path.exists(f"pytharg/static/{self.project}/img/{name}") == True:
						if os.path.getsize(f"pytharg/static/{self.project}/img/{name}") >= \
						os.path.getsize(f"{os.path.join(root, name)}"):
							print(f"duplicated image found: '{name}'")
							pass
						else:
							os.remove(f"pytharg/static/{self.project}/img/{name}")
							shutil.copy(f"{os.path.join(root, name)}", f"pytharg/static/{self.project}/img/{name}")
							img.append(name)
					else:
						img.append(name)
						shutil.copy(f"{os.path.join(root, name)}", f"pytharg/static/{self.project}/img/{name}")	
	...
	def send_base_html(self):
		with open("new_base.html", "w", encoding="utf-8") as base:
			soup = BeautifulSoup(open(f"pytharg/templates/{self.project}/main/index.html"), features="html.parser")
			head = soup.find("head")
			...
			for link in head.find_all("link"):
				...
				if file.endswith(".css"):
					link['href'] = f'{get_static_link(file=file, folder=f"{self.project}/css")}'					
					...
					link["href"] = f'{get_static_link(file=file, folder=f"{self.project}/img")}'
			...
			base.write(get_include_strin(f"{self.project}", partial="header"))
			base.write("\n\n")
			...
			base.write(get_include_strin(f"{self.project}", partial="footer"))
			base.write("\n\n")
			base.write("\n <!-- Javascript Files -->\n")
			for script in soup.find_all("script"):
				if (script["src"].endswith(".js")) and ("https" not in script['src']):
					file = script["src"].split("/")[-1:][0]
					script["src"] = f'{get_static_link(file=file, folder=f"{self.project}/js")}'
		...
		with open(f"pytharg/templates/{self.project}/base.html", "w", encoding="utf-8") as base2:
...
if __name__ == "__main__":
	BTemplate(template="Avilon", project="avilon")